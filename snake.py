...
class Snake:
    ...
    w_edge = 0.1  # The weight to the edge energy.
    w_term = 0.5  # The weight to the term energy.

    #################################
    # Constructor
    #################################
    def __init__(self, image, init_binary, init_snake, closed=True):
        """
        Object constructor
        ...
        if self.closed:
            self.points = np.squeeze(init_snake[0])
    ...
    @staticmethod
    def f_curvature(p, prev, next_p):
        """
        The Curvature energy
        :param p: The point being analysed
        :param prev: The previous point in the curve
        :param next_p: The next point in the curve
    ...
    def f_conf(p, prev, next_p):
        ...
        :param next_p: The next point on the snake
    ...
    def add_missing_points(self):
        ...
        for i in range(0, snake_size):
            ...
            curr = self.points[i]
            next_p = self.points[(i + 1) % snake_size]
            next_p2 = self.points[(i + 2) % snake_size]

            if Snake.dist(curr, next_p) > self.MAX_DISTANCE_BETWEEN_POINTS:
                # Pre-computed uniform cubig b-spline for t = 0.5
                c0 = 0.125 / 6.0
                c1 = 2.875 / 6.0
                c2 = 2.875 / 6.0
                c3 = 0.125 / 6.0
                x = prev[0] * c3 + curr[0] * c2 + next_p[0] * c1 + next_p2[0] * c0
                y = prev[1] * c3 + curr[1] * c2 + next_p[1] * c1 + next_p2[1] * c0
    ...
    def step(self):
        ...
        for i in range(0, len(self.points)):
            ...
            for dx in range(-hks, hks):
                for dy in range(-hks, hks):
                    ...
                    e_curvature[dx + hks][dy + hks] = self.f_curvature(p, prev, next_p)
                    e_line[dx + hks][dy + hks] = self.f_line(p)
                    e_edge[dx + hks][dy + hks] = self.f_edge(p)
                    e_term[dx + hks][dy + hks] = self.f_term(p, prev, next_p)
                    e_conf[dx + hks][dy + hks] = self.f_conf(p, prev, next_p)
        ...
        # Post treatment to the snake, remove overlaping points and
        # add missing points
    ...
    def set_w_line(self, x1):
        """
        Utility function (used by cvCreateTrackbar) to set the value of w_line.
        :param x1: The new value of w_line (scaled by 100)
        """
        self.w_line = x1

    def set_w_edge(self, x2):
        self.w_edge = x2

    def set_alpha(self, x3):
        self.alpha = x3

    def set_beta(self, x4):
        self.alpha = x4

    def update_binary(self, new_binary):
